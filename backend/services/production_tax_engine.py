...
class ProductionTaxEngine:
    ...
    def _get_active_ruleset_payload(self, key: str, on_date: Optional[date] = None) -> Optional[dict]:
        """Fetch active ruleset payload from DB.

        Falls back to None if ruleset not present or DB not available in context.
        """
    ...
    def _get_federal_withholding_rules(self, on_date: Optional[date] = None) -> Optional[dict]:
        return self._get_active_ruleset_payload('irs_federal_withholding', on_date=on_date)

    def _get_federal_standard_deduction(self, filing_status: str, on_date: Optional[date] = None) -> float:
        rules = self._get_federal_withholding_rules(on_date=on_date)
        if rules and isinstance(rules.get('standard_deduction'), dict):
            return float(rules['standard_deduction'].get(filing_status, self.STANDARD_DEDUCTION_2024.get(filing_status, 14600)))
        return float(self.STANDARD_DEDUCTION_2024.get(filing_status, 14600))

    def _get_federal_brackets(self, filing_status: str, on_date: Optional[date] = None) -> List[Tuple[float, float]]:
        rules = self._get_federal_withholding_rules(on_date=on_date)
        if rules and isinstance(rules.get('brackets'), dict):
            status_brackets = rules['brackets'].get(filing_status)
            if isinstance(status_brackets, list) and status_brackets:
                converted: List[Tuple[float, float]] = []
                for bracket in status_brackets:
                    rate = float(bracket['rate'])
                    max_value = bracket.get('max')
                    limit = float('inf') if max_value is None else float(max_value)
                    converted.append((limit, rate))
                return converted
    ...
    def _get_fica_rules(self, on_date: Optional[date] = None) -> Optional[dict]:
        return self._get_active_ruleset_payload('us_fica', on_date=on_date)
    ...
    def calculate_federal_income_tax(
        ...
        annual_wages = gross_wages * self._get_pay_periods(pay_frequency)
        ...
        rules_on_date = date.today()

        standard_deduction = self._get_federal_standard_deduction(filing_status, on_date=rules_on_date)

        adjusted_wages = annual_wages + other_income - deductions - standard_deduction
        adjusted_wages = max(0, adjusted_wages)

        brackets = self._get_federal_brackets(filing_status, on_date=rules_on_date)
        annual_tax = self._calculate_bracket_tax(adjusted_wages, brackets)

        annual_tax = max(0, annual_tax - dependents_credit)

        per_period_tax = annual_tax / self._get_pay_periods(pay_frequency)
    ...
    def calculate_social_security(self, gross_wages: float, ytd_ss_wages: float = 0) -> Dict:
        ...
        rules_on_date = date.today()
        fica = self._get_fica_rules(on_date=rules_on_date) or {}

        wage_base = float(
            fica.get('social_security', {}).get('wage_base', self.SOCIAL_SECURITY_WAGE_BASE_2024)
        )
        ss_rate = float(
            fica.get('social_security', {}).get('rate', self.SOCIAL_SECURITY_RATE)
        )
        ...
        employer_tax = taxable_wages * ss_rate
    ...
    def calculate_medicare(self, gross_wages: float, ytd_medicare_wages: float = 0) -> Dict:
        ...
        medicare_rate = float(fica.get('medicare', {}).get('rate', self.MEDICARE_RATE))
        additional_rate = float(fica.get('medicare', {}).get('additional_rate', self.ADDITIONAL_MEDICARE_RATE))
        additional_threshold = float(fica.get('medicare', {}).get('additional_threshold', self.ADDITIONAL_MEDICARE_THRESHOLD))

        regular_medicare = gross_wages * medicare_rate

        additional_medicare = 0
        total_wages = ytd_medicare_wages + gross_wages

        if total_wages > additional_threshold:
            if ytd_medicare_wages >= additional_threshold:
                additional_wages = gross_wages
            else:
                additional_wages = total_wages - additional_threshold

            additional_medicare = additional_wages * additional_rate

        employee_tax = regular_medicare + additional_medicare
        employer_tax = gross_wages * medicare_rate
    ...
    def calculate_suta(
        ...
        """Calculate state unemployment insurance tax (employer only)."""
        state = state.upper()
    ...
    def calculate_paid_family_leave(
        ...
        """Calculate paid family leave tax."""
        state = state.upper()
